...
bool TSection::Init(TGSociety *_Society, TGObject* _OurObject,bool Oldest)
     ...
     MaxGeneration              = log10(min(MAP_WIDTH,MAP_HEIGHT))/log10(2);
     ScionsNumber               = 0;
...
bool TSection::FingerAllSections(bool EInit, bool CheckOnAB,bool FindObject/*,TCanvas* Canvas*/)
     ...
     IList.TotalNum = pow(2,(OldestSection->MaxExistingGeneration+1)*2);
...
bool TSection::DelGeneration(int G)
     ...
     List.TotalNum = ceil((pow(2,(OldestSection->MaxExistingGeneration - Generation)*2) - 2)/log(2));
     List.Num      = 0;
...
bool TSection::SplitSection()
     ...
     GList.TotalNum = pow(2,(OldestSection->MaxExistingGeneration+1)*2);
...
TSection::TGList TSection::GetAllNeighbour(/*TCanvas *Canvas*/)
        ...
        SList.TotalNum   = 4*pow(2,OldestSection->MaxExistingGeneration - Generation) + 4;
...
TSection::TGList TSection::GetDaughterNeighbour(TSection *_Section)
        ...
        BList.TotalNum = 4*/*ceil*/(pow(2,_Section->Generation - Generation)) + 4;
...
//            через ту, которой она уже была залита (или равен) - результат отрицательный
           {
           if ((Steps + sqrt(kvdrt(_Section->X - X)+kvdrt(_Section->Y - Y))) > _Section->Steps)
              return false;
...
//        _Section->Steps = Steps + ( (_Section->X - X)^2 + (_Section->Y - Y)^2)^(1/2);
        _Section->Steps = Steps + sqrt(kvdrt(_Section->X - X)+kvdrt(_Section->Y - Y));
...
float R      = sqrt(kvdrt(DeltaX)+kvdrt(DeltaY));
...
bool TTarget::GetTarget(int _X,int _Y,float _Z)
     ...
     DY       = Y - StartY;
     DZ       = Z - StartZ;
     Angle    = (DX!=0)?atan2(DY,DX):(Pi/2);

     Distance = sqrt(kvdrt(DX)+kvdrt(DY));
     WentWay  = 0;
     ...
     SpeedOXY =  sqrt(kvdrt(Speed)/(1+fkvdrt(StartZTangens)));
...
void TTarget::GetCurXYZ()
     ...
     WentWay  = sqrt(fkvdrt(CurrentX-StartX)+fkvdrt(CurrentY-StartY));
        ...
        for (int y = round(-Radius);y<=round(Radius);y++)
         for (int x = round(-Radius);x<=round(Radius);x++)
          ...
          if ((Society->MAP[x+_X][y+_Y].Team!=-1)&&(Society->MAP[x+_X][y+_Y].Team!=NUMBER_OF_TEAMS))
             ...
             Society->GObject[Team][Index].Live-=damage;
...
// Points.Init(this);
 ...
 for (int t = 0;t<NUMBER_OF_TEAMS;t++)
    {
    IncludedInBase[t] = new TIncInBase [NUMBER_OF_UNITS];
...
void TGObject::GetGlobalPos(int fX,int fY,int WhoCommand)
     ...
     NewPosition.GlobalAngle = StdAngl(atan2(fY-Y,fX-X)/OGIR);
     NewPosition.GlobalR = sqrt(kvdrt(fY-Y) + kvdrt(fX-X));
     ...
     if(NewPosition.GlobalX >=0)
        ...
        if ( (GlobalDeltaX!=0)||(GlobalDeltaY!=0) ) // если есть куда идти...
           ...
           if (GlobalDeltaX!=0) // во избежании деления на нуль
              NewPosition.GlobalAngle = StdAngl(atan2(GlobalDeltaY,GlobalDeltaX)/OGIR);
              else  {
                    if (GlobalDeltaY > 0)
              ...
              NewPosition.GlobalR =  sqrt ( kvdrt(GlobalDeltaX)+kvdrt(GlobalDeltaY) );
...
//     if (Enemy == NULL) {ShowMessage("НЕВЕРНО ВЫЗВАНА Ф-я GetEnemy()!!"); return;};
     ...
     PrimEnemy.R = sqrt(kvdrt(DeltaX)+kvdrt(DeltaY));
     PrimEnemy.Angle = StdAngl(atan2(DeltaY,DeltaX)/OGIR);
     ACTION = ATTACK_ACT;
     }
...
void TGGroup::GetNewPos(int X,int Y)
     ...
     */
     ...
     float
         R = sqrt(kvdrt(DeltaY) + kvdrt(DeltaX)),
         Angle =(DeltaX!=0)?StdAngl(atan2(DeltaY,DeltaX)/OGIR):((DeltaY>0)?90:(-90));
         //суть условия такова
     ...
     for (int i = 0;i<Number;i++)
           ...
           Units[i].PUnit->SectionWay.FindWay();
...
//           Units[i].PUnit->GetGlobalPos(R,Angle,MAN);
...
bool TGSociety::LoadConfig()
     ...
     for (int t = 0; t<NUMBER_OF_TEAMS; t++)
         ...
         for (int i = 0; i<NUMBER_OF_UNITS; i++)
             fread(&GObject[t][i],OBJ_SIZE,1,f);
         }
     ...
     //  10.2.1. Очистка памяти выделенной под команды
     if (Teams!= NULL)
        {
        delete[] Teams;
        Teams = NULL;
     ...
     for (int t = 0; t < NUMBER_OF_TEAMS; t++)
...

      {
       ...
       for (int t = 0; t<NUMBER_OF_TEAMS;t++)    //группы
          {
          Groups[t] = new TGGroup [NUMBER_OF_UNITS];          
...
void TGSociety::Sinchronize(int Team,int Index, bool SnAction)
    {
      int X1 = GObject[Team][Index].X-(GObject[Team][Index].Width/2),
          X2 = GObject[Team][Index].X+(GObject[Team][Index].Width/2),
          Y1 = GObject[Team][Index].Y-(GObject[Team][Index].Width/2),
          Y2 = GObject[Team][Index].Y+(GObject[Team][Index].Width/2);
                   ...
                   for(int i = INDEX;i<NUMBER_OF_UNITS;i++)
                      ...
                      if ((R <= GObject[T][I].Sight)&& (GObject[T][I].NVO < MAX_SIGHT)&&GObject[T][I].Thinking) //если внешний объект в поле зрения данного
                         {
                             ...
                             if(R!=0)
                               {
                                 if (DeltaX!=0)
                                 Angle = StdAngl((float)atan2(DeltaY,DeltaX)/OGIR);//вычисляем угол
                      ...
                      if ((GObject[t][i].Sight >= R)&&(GObject[t][i].NVO<max_SIGHT)&&GObject[t][i].Thinking)
                           ...
                           if (Angle == 1000)
                              {
                              if (DeltaX!=0)
                                Angle = StdAngl(atan2(DeltaY,DeltaX)/OGIR);//вычисляем угол
                                    else if(DeltaY > 0) Angle = 90;
                                         else if(DeltaY < 0)Angle = 270;
                              }
...
bool TGSociety::Check(float Angle,int Team,int Index,int &Left_Zero_Right,float R)
     {
     float AngleOW =StdAngl(atan2((GObject[Team][Index].Width+1),SmalestR(Team,Index))/OGIR);
     int i;
     bool result = true;
...
bool TGSociety::CheckPoint (TGObject *Unit, float Angle,bool WithAngle,int R,int I)
     ...
     bool Result = true;
       ...
       float WentR      = (fabs(DeltaX)>fabs(DeltaY))?sqrt(kvdrt(0) + kvdrt(Unit->Y - SmallPoint)):sqrt(kvdrt(0) + kvdrt(Unit->X - SmallPoint));
       ...
       WentR      = (fabs(DeltaX)>fabs(DeltaY))?sqrt(fkvdrt(i) + fkvdrt(Unit->Y - SmallPoint)):sqrt((fkvdrt(i) + fkvdrt(Unit->X - SmallPoint)));
       }
...
void TGSociety::SetPos(int Team,int Index/*, TCanvas *Canvas*/)   // Алгоритм сложного перемещения
     ...
     {/* if
      ...
      for (float A = _Angle; A<360+_Angle;A+=360/PART_IN_CYRCLE)
          ...
          while (!CheckPoint(&GObject[Team][Index],GObject[Team][Index].NewPosition.Angle,false,0,0))
                {
                float ToAngle = SetNearestAngle(Angles,N,Angle,_Angle,Speeds,Speed);
                GObject[Team][Index].GetNewPos(ToAngle,float(Speed+1));
                }
...
/*     if((GObject[Team][Index].NewPosition.GlobalX > 0) && GObject[Team][Index].Is)
     ...
     float AngleOW = atan2(OBJECT_WIDTH,SmlstR);
...
int TGSociety::Analis_Change(int Team,int Index,int index)
    {
    /*
    Если выражаться груба то эта функция работает так:
    ...
    */
    ...
    int Enemy=0;//оценка опасонсти группы врагов в баллах. (пока это просто количество)
    ...
    if ((Ally*GObject[Team][Index].N) >= Enemy)
...
void TGSociety::Dye(TGObject *Unit)
     {
     Dead++;
     if ((Unit->TeamIndex == 4)&&(Unit->Index == 17))
        bool Stop = true;
     ...
     const Team        = Unit->TeamIndex;//запоминаем его данные для последствующего использования
     ...
     Unit->PrimaryEnemy   = NULL;
        ...
        int X1 = X-round(GObject[Team][Index].Width/2),
            X2 = X+round(GObject[Team][Index].Width/2),
            Y1 = Y-round(GObject[Team][Index].Width/2),
            Y2 = Y+round(GObject[Team][Index].Width/2);
        for (int y = Y1;y<=Y2;y++) //пока он представляет заштрихованный квадрат на карте.
         for (int x = X1;x<=X2;x++)
         if (
            (InSpace(0,MAP_WIDTH,x,true,false))
...
void TGSociety::SendSelectedUnits (int Team,short int LOG_DATA, TGGroup::TDATA DATA)
     ...
     for (int i = 0; i<NUMBER_OF_UNITS;i++)
         ...
         if ( (Group->Mission.LastDataSend.XY.X != DATA.XY.X) || (Group->Mission.LastDataSend.XY.Y != DATA.XY.Y) )
            CanReturn = false;
