...
bool SetTrajTangens (float g,float Speed, float H, float D,bool Roof,float &Tangens)
     ...
     float Angle = ((fkvdrt(Speed)*H)-((-g)*fkvdrt(D))) / (fkvdrt(Speed)*sqrt( (double)fkvdrt(H )+fkvdrt(D)));
     if (fabs(Angle)>1)
        return false;
...
float GetZ (float Y0,float Speed,float g,float X,float StartTangens)
           {
           float SpeedOX      = sqrt( (double)fkvdrt(Speed )/(float(1)+fkvdrt(float(StartTangens))));
